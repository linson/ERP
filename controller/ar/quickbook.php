<?php
class ControllerArQuickbook extends Controller{
	private $error = array();
	public function index() {
		...
		$this->load->language('ar/quickbook');
		$this->document->title = 'Quickbook import';
...
/*
    	if('ASCII' == $enc){
    	  $filename = iconv('ASCII','utf-8//TRANSLIT//IGNORE',$filename);
    	}
...
*/
        ...
        //$this->log->aPrint( $content );
			...
			if($content){
				$this->model_ar_quickbook->csvImport($filename);
				$this->session->data['success'] = $this->language->get('text_success');
				$this->redirect(HTTPS_SERVER . 'index.php?route=ar/quickbook&token=' . $this->session->data['token']);
			} else {
		...
		if (isset($this->session->data['success'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'index.php?route=ar/quickbook&token=' . $this->session->data['token'],
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);

		$this->data['restore'] = HTTPS_SERVER . 'index.php?route=ar/quickbook&token=' . $this->session->data['token'];
		$this->data['csv'] = HTTPS_SERVER . 'index.php?route=ar/quickbook/csv&token=' . $this->session->data['token'];
		$this->data['csv_import'] = HTTPS_SERVER . 'index.php?route=ar/quickbook&token=' . $this->session->data['token'];
		$this->data['csv_export'] = HTTPS_SERVER . 'index.php?route=ar/quickbook/csvExport&token=' . $this->session->data['token'];

		$this->load->model('ar/quickbook');
		$this->data['tables'] = $this->model_ar_quickbook->getTables();

		$this->template = 'ar/quickbook.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'ar/quickbook')) {
