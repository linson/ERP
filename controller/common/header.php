  	...
  	$this->data['base'] = (HTTPS_SERVER) ? HTTPS_SERVER : HTTP_SERVER;
   	...
   	$this->data['text_localisation'] = $this->language->get('text_localisation');
		...
		if (!$this->user->isLogged() || !isset($this->session->data['token']) ) {
			...
			$this->data['lnk_product'] = HTTPS_SERVER . 'index.php?route=catalog/product&token=' . $this->session->data['token'];
			...
			$this->data['lnk_material_lookup'] = HTTPS_SERVER . 'index.php?route=material/lookup&token=' . $this->session->data['token'];
			$this->data['lnk_material_productpackage']  = HTTPS_SERVER . 'index.php?route=material/productpackage&token=' . $this->session->data['token'];
			$this->data['lnk_material_history']  = HTTPS_SERVER . 'index.php?route=material/history&token=' . $this->session->data['token'];
			$this->data['lnk_material_order']  = HTTPS_SERVER . 'index.php?route=material/order&token=' . $this->session->data['token'];
			
			$this->data['lnk_product_price'] = HTTPS_SERVER . 'index.php?route=product/price&token=' . $this->session->data['token'];
			$this->data['lnk_product_inventory'] = HTTPS_SERVER . 'index.php?route=product/inventory&token=' . $this->session->data['token'];
			$this->data['lnk_product_oem'] = HTTPS_SERVER . 'index.php?route=product/oem&token=' . $this->session->data['token'];

			$this->data['lnk_sales_lookup'] = HTTPS_SERVER . 'index.php?route=sales/list&token=' . $this->session->data['token'];
			$this->data['lnk_sales_order']  = HTTPS_SERVER . 'index.php?route=sales/order&token=' . $this->session->data['token'];
			$this->data['lnk_sales_stat']  = HTTPS_SERVER . 'index.php?route=sales/stat&token=' . $this->session->data['token'];
			$this->data['lnk_sales_calendar']  = HTTPS_SERVER . 'index.php?route=sales/calendar&token=' . $this->session->data['token'];
			$this->data['lnk_update_promotion']  = HTTPS_SERVER . 'index.php?route=sales/promotion&token=' . $this->session->data['token'];
		}
		
		$this->id       = 'header';

    if( isset($this->request->get['mobile']) ){
  		$this->template = 'common/mobile/header.tpl';
  	}else{
