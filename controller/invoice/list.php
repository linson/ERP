<?php
class ControllerInvoiceList extends Controller {
  ...
  public function print_ship(){
    ...
    $txid = isset($this->request->get['txid']) ? $this->request->get['txid'] : '';
    if($txid == ''){}
    if($this->model_invoice_list->print_ship($txid)){
  ...
  public function getList(){
    ...
    $url = '';
  	...
  	if(isset($this->request->get['filter_ship_date'])){
				$url .= '&filter_ship_date=' . $this->request->get['filter_ship_date'];
		    $filter_ship_date = $this->request->get['filter_ship_date'];
  	}else{
  	    $filter_ship_date = NULL;  
  	}
    ...
    # link
		...
		$this->data['lnk_delete'] = HTTP_SERVER . 'index.php?route=invoice/list/delete&token=' . $this->session->data['token'] . $url;
    ...
    $request = array(
      'filter_txid'	  => $filter_txid,
			...
			'filter_ship_date'=> $filter_ship_date,
    ...
    $total = $this->model_invoice_list->getTotalList($request);
		$response = $this->model_invoice_list->getList($request);
    ...
    //$this->log->aPrint( $this->data['txs'] );
		...
		$pagination->url = HTTP_SERVER . 'index.php?route=invoice/list&token=' . $this->session->data['token'] . $url . '&page={page}';

		$this->data['pagination'] = $pagination->render();

		$this->data['filter_txid']       = $filter_txid;
		$this->data['filter_store_name'] = $filter_store_name;
		$this->data['filter_ship_date']  = $filter_ship_date;
    ...
    # call view
    $this->template = 'invoice/list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  ...
  public function delete(){
		...
		$this->redirect(HTTP_SERVER . 'index.php?route=invoice/list&token=' . $this->session->data['token']);
   	$this->getList();
 	}
