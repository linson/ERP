...
<table class='detailSum'>
  ...
  foreach($a30 as $k => $h){
    $aTxid = explode('-',$k);
    $displayTxid = $aTxid[1] . '-' . $aTxid[2];
    $odate = $this->util->date_format_us($h['order_date'],'/');
  ?>
  <tr>
    <td colspan=4>
      <table border=0 cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td></td>
          <td>
            <?php echo $this->util->date_format_us($hPay['pay_date'],'/') . " ::" . $hPay['pay_price'] . " :: " . $hPay['pay_method'] . " ::" . $hPay['pay_num']; ?>
          </td>
        </tr>
        <?php } ?>
        <?php
