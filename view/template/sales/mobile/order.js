$(document).ready(function(){
  ...
  $.fn.managePromotion = function($model,$cnt){
    ...
    if( 'W' == $storetype ){
      if( ('3S8623' == $model && $cnt >= 50) || ('3S8627' == $model && $cnt >= 50) ){
        $msg = '[Promotion A] WWW 2oz Buy 50 Get 5 Free \n';
        $ptn = /(\[Promotion A\] WWW.+\n?)/;
    ...
    if( 'R' == $storetype ){
      ...
      }else if($order_price >= 500){
        $msg = '[Promotion A] over $500 , Lace + Spritz (4oz) \n';
    ...
    if( ('VN5004' == $model && $cnt > 0) || ('VN5008' == $model && $cnt > 0) || ('VN5018' == $model && $cnt > 0) ||
        ('VN5024' == $model && $cnt > 0) || ('VN5026' == $model && $cnt > 0) || ('VN5014' == $model && $cnt > 0) ){
      $msg = '[Promotion B] B/T 2LB Gel -> 1LB Free \n';
      $ptn = /(^\[Promotion B\] B\/T.+\n?)/;
      if( $el_desc.val().match($ptn) )  $el_desc.val( $el_desc.val().replace($ptn,'') );
      if( $added.match($ptn) )  $added = $added.replace($ptn,'');
      $added = $added + $msg;
    ...
    if( 'VN7916' == $model && $cnt >= 2){
      $ptn = /(^\[Promotion B\] Edge.+\n?)/;
      $msg = '[Promotion B] Edge Gel Display Buy 2 Get 1 Free \n';
      if( $el_desc.val().match($ptn) )  $el_desc.val( $el_desc.val().replace($ptn,'') );
      if( $added.match($ptn) )  $added = $added.replace($ptn,'');
      $added = $added + $msg;
    }
