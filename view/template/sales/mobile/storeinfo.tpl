...
/***
  ...
  if(!$description){
    if(isset($store_dc)){
      ...
      $i = 1;
      $description .= "===== Store Discount =====" . chr(13);
      foreach($aDC as $desc => $dc){
        // todo. textarea won't be enlarged automatically. need to check later
        if("" != $dc) $description .= $dc . '% : ' . $desc . chr(13);
      }
    }
  }
***/
//$dc1 = $dc2 = 0;  // define store discount
//description hard coding for promotion
//$description = "";
$descLen = explode(chr(13),$description);
$descHeight = count( $descLen ) * 12;
...
//              $this->log->aPrint( $aDC ); exit;
            ...
            <tr>
              <td colspan=4>
                <textarea name='description' style='min-height:100px;width:98%;<?php echo $descHeight; ?>;scroll-y:auto;'><?php echo $description; ?></textarea>
              </td>
            </tr>
          </table>
          <?php
...
$('document').ready(function(){
  ...
  ***/
  ...
  $('textarea[name=description]').css('overflow-y','auto');
});
</script>