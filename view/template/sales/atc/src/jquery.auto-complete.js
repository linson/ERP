...
var
	...
	AutoComplete = $.autoComplete = {
		...
		defaults: {
			...
			// Server Script Path
			ajax: 'view/template/sales/atc/ajax.php',
	...
	AutoCompleteFunction = function( self, options ) {
        ...
        var $prt = $tgt.parent('ul');
        $prt.children().each(function(idx,child){
          if(child.className == 'auto-complete-list-rollover'){
            var $fallbackIdx = idx;  
            liData = currentList[idx];
          }
        });
        
        // it can be or cannot be solution , besso-201103 !!!!!!
        /*
        if(liData == null){
          debugger;
          liData = currentList[$fallbackIdx];
        }
        **/
        if($tgt.is('li.auto-complete-list-rollover')){
  			  ...
  			  $input.val( inputval === separator ? 
  			  	//inputval.substr( 0, inputval.length - inputval.split( separator ).pop().length ) + liData.value + separator :	liData.value 
  			  	inputval.substr(0, inputval.length - inputval.split( separator ).pop().length ) + liData.model + separator :	liData.model );
  			...
  			//else{}  // if user click the other area
			...
			// assume no external one , besso-201103 
			/**
...
/*
    var $mama = $tgt.parents('table');
      var $cat = $mama.attr('id');
      var $atcObject = $mama.find('input.atc');
*/
			settings.postData[ settings.postVar ] = cache.val;
			settings.postData['cat'] = settings.cat;
			xhr = $.ajax({
    ...
    function queryProduct(){
      var model = $input.attr('value');

      var xhr = $.ajax({
				type: 'GET',
				url: 'index.php?route=sales/atc/getProductBatchProxy',
				//url: 'index.php?route=sales/atc/getProductBatchProxy',
				cache: $.ajaxSettings.cache,
				dataType: 'json',
				data: 'model=' + model,
				success: function( list ) {
				  var node = $input.parents('tr');
            ...
            if(idx != 0){
              ...
              // if it's first row
              node.find('input[name="model[]"]').val();
              node.find('.preview').css('display','inline');
              //debugger;
              $.each(line, function(key,val){
                if(key == 'locked' && val == true){
                  // dhtml add L button to quantity
                  // <a class='check_locked' style='font-weight:bold;font-size:14px;color:white;'>L</a>
                  node.find('.check_locked').css('display','inline');
                }
              });
            }
            ...
            $.each(line, function(key,val){
              ...
              if(key == 'model'){
                node.find('input[name="model[]"]').val(val);
              }
              if(key == 'product_id'){
                node.find('input[name="product_id[]"]').val(val);
              }
              if(key == 'weight'){
                node.find('input[name="weight[]"]').val(val);
              }
              if(key == 'quantity'){
                node.find('input[name="stock[]"]').val(val);
              }
              if(key == 'dc'){
                node.find('input[name="discount[]"]').val(val);
              }
              if(key == 'dc2'){
                node.find('input[name="discount2[]"]').val(val);
              }
              ...
              if(storetype == 'W'){
                if(key == 'ws_price'){
                  node.find('input[name="price[]"]').val(val);
                }
              }else{
                if(key == 'rt_price'){
                  node.find('input[name="price[]"]').val(val);
              ...
              if(key == 'image'){
                ...
                //node.find('input[name="image[]"]').val(imgUrl + val);
                //var $img = node.find('input[name="image[]"]').attr('value');
                node.find('.preview').attr('title',$imgUrl);
                node.find('.preview').css('display','inline');
              }
              if(key == 'product_name'){
                node.find('.product_name').html(val);
              }
              if(key == 'pc'){
                  node.find('.pc').html(val);
              }
            });
          });
          // focus to first QTY box , besso-201103
          $input.parents('tr').find('input[name="cnt[]"]').select();
				},
				error: function() {
					// need to good one
				}
