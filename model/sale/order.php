...
class ModelSaleOrder extends Model {
	public function deleteOrder($order_id){
		$order_query = $this->db->query("SELECT * FROM transaction WHERE order_id = '" . (int)$order_id . "'");
		...
		$this->db->query("DELETE FROM transaction WHERE order_id = '" . (int)$order_id . "'");
	...
	public function addProduct($order_id, $data){
		...
		$this->db->query("UPDATE transaction SET total = '" . (float)$data['new_grand_total'] . "' WHERE order_id = '" . (int)$order_id . "'");
	...
	public function updateShippingAddress($order_id, $data){
		$this->db->query("UPDATE transaction SET shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', shipping_company = '" . $this->db->escape($data['shipping_company']) . "', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', shipping_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', shipping_city = '" . $this->db->escape($data['shipping_city']) . "', shipping_postcode = '" . $this->db->escape($data['shipping_postcode']) . "', shipping_zone = '" . $this->db->escape($data['shipping_zone']) . "', shipping_zone_id = '" . (int)$data['shipping_zone_id'] . "', shipping_country = '" . $this->db->escape($data['shipping_country']) . "', shipping_country_id = '" . (int)$data['shipping_country_id'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
	}

	public function updatePaymentAddress($order_id, $data){
		$this->db->query("UPDATE transaction SET payment_firstname = '" . $this->db->escape($data['payment_firstname']) . "', payment_lastname = '" . $this->db->escape($data['payment_lastname']) . "', payment_company = '" . $this->db->escape($data['payment_company']) . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', payment_address_2 = '" . $this->db->escape($data['payment_address_2']) . "', payment_city = '" . $this->db->escape($data['payment_city']) . "', payment_postcode = '" . $this->db->escape($data['payment_postcode']) . "', payment_zone = '" . $this->db->escape($data['payment_zone']) . "', payment_zone_id = '" . (int)$data['payment_zone_id'] . "', payment_country = '" . $this->db->escape($data['payment_country']) . "', payment_country_id = '" . (int)$data['payment_country_id'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
	}

	public function addOrderHistory($order_id, $data){
		$this->db->query("UPDATE transaction SET order_status_id = '" . (int)$data['order_status_id'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");

		if($data['append']){
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$data['order_status_id'] . "', notify = '" . (isset($data['notify']) ? (int)$data['notify'] : 0) . "', comment = '" . $this->db->escape(strip_tags($data['comment'])) . "', order_date = NOW()");
		}

      if($data['notify']){
       	$order_query = $this->db->query("SELECT *, os.name AS status FROM transaction x LEFT JOIN " . DB_PREFIX . "order_status os ON (x.order_status_id = os.order_status_id AND os.language_id = x.language_id) LEFT JOIN " . DB_PREFIX . "language l ON (x.language_id = l.language_id) WHERE x.txid = '" . (int)$order_id . "'");
			...
			if($order_query->num_rows){
				...
				$message .= $language->get('text_date_added') . ' ' . date($language->get('date_format_short'), strtotime($order_query->row['order_date'])) . "\n\n";
    ...
    /**
		$sql = "SELECT x.txid, CONCAT(x.firstname, ' ', x.lastname) AS name, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = x.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, x.order_date, x.total, x.currency, x.value FROM transaction x";
		if(isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])){
			$sql .= " WHERE x.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
		} else {
			$sql .= " WHERE x.shipped_yn = 'Y'";
		}

		if(isset($data['filter_order_id']) && !is_null($data['filter_order_id'])){
			$sql .= " AND x.txid = '" . (int)$data['filter_order_id'] . "'";
		}

		if(isset($data['filter_name']) && !is_null($data['filter_name'])){
			$sql .= " AND CONCAT(x.firstname, ' ', x.lastname) LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}

		if(isset($data['filter_order_date']) && !is_null($data['filter_order_date'])){
			$sql .= " AND DATE(x.order_date) = DATE('" . $this->db->escape($data['filter_order_date']) . "')";
		}

		if(isset($data['filter_total']) && !is_null($data['filter_total'])){
			$sql .= " AND x.total = '" . (float)$data['filter_total'] . "'";
		}
	...
	public function generateInvoiceId($order_id){
		$query = $this->db->query("SELECT MAX(invoice_id) AS invoice_id FROM transaction");
		...
		$this->db->query("UPDATE transaction SET invoice_id = '" . (int)$invoice_id . "', invoice_prefix = '" . $this->db->escape($this->config->get('config_invoice_prefix')) . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
	...
	public function getOrderHistory($order_id){
		$query = $this->db->query("SELECT oh.order_date, os.name AS status, oh.comment, oh.notify FROM " . DB_PREFIX . "order_history oh LEFT JOIN " . DB_PREFIX . "order_status os ON oh.order_status_id = os.order_status_id WHERE oh.order_id = '" . (int)$order_id . "' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY oh.order_date");
	...
	public function getOrderTax($product_id, $order_id){ // TU
		$query = $this->db->query("SELECT tr.rate AS rate, tr.description, tr.priority FROM " . DB_PREFIX . "tax_rate tr LEFT JOIN " . DB_PREFIX . "zone_to_geo_zone z2gz ON (tr.geo_zone_id = z2gz.geo_zone_id) LEFT JOIN " . DB_PREFIX . "geo_zone gz ON (tr.geo_zone_id = gz.geo_zone_id) WHERE (z2gz.country_id = '0' OR z2gz.country_id = (SELECT payment_country_id FROM transaction WHERE order_id = '" . (int)$order_id . "')) AND (z2gz.zone_id = '0' OR z2gz.zone_id = (SELECT payment_zone_id FROM transaction WHERE order_id = '" . (int)$order_id . "')) AND tr.tax_class_id = (SELECT tax_class_id FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "')");
	...
	public function getTotalOrdersByStoreId($store_id){
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM transaction WHERE store_id = '" . (int)$store_id . "'");

		return $query->row['total'];
	}

	public function getOrderHistoryTotalByOrderStatusId($order_status_id){
	  	$query = $this->db->query("SELECT oh.order_id FROM " . DB_PREFIX . "order_history oh LEFT JOIN transaction x ON (oh.order_id = x.txid) WHERE oh.order_status_id = '" . (int)$order_status_id . "' AND x.shipped_yn = 'Y' GROUP BY order_id");

		return $query->num_rows;
	}

	public function getTotalOrdersByOrderStatusId($order_status_id){
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM transaction WHERE order_status_id = '" . (int)$order_status_id . "' AND shipped_yn = 'Y'");

		return $query->row['total'];
	}

	public function getTotalOrdersByLanguageId($language_id){
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM transaction WHERE language_id = '" . (int)$language_id . "' AND shipped_yn = 'Y'");

		return $query->row['total'];
	}

	public function getTotalOrdersByCurrencyId($currency_id){
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM transaction WHERE currency_id = '" . (int)$currency_id . "' AND shipped_yn = 'Y'");

		return $query->row['total'];
	}

  # Order
	public function getTotalOrderToday($user = ''){
	  $sql = "SELECT SUM(order_price) AS total, SUM(payed_sum) AS payed, SUM(balance) AS balance  FROM transaction WHERE substr(order_date,1,10) = substr(now(),1,10)";
	  if($user) $sql.= " and order_user = '$user'";
	  echo $sql;
   	$query = $this->db->query($sql);
		return $query->row;
	}

	public function getTotalOrderYday($user = ''){
	  $sql = "SELECT SUM(order_price) AS total, SUM(payed_sum) AS payed, SUM(balance) AS balance FROM transaction WHERE substr(order_date,1,10) = adddate(substr(now(),1,10),-1)";
	  if($user) $sql.= " and order_user = '$user'";
  	$query = $this->db->query($sql);
		return $query->row;
	}

	public function getTotalOrderThisMonth($user = ''){
	  $sql = "SELECT substr(order_date,6,2) as thismonth ,SUM(order_price) AS total, SUM(payed_sum) AS payed, SUM(balance) AS balance FROM transaction WHERE substr(order_date,6,2) = substr(now(),6,2)";
	  //$sql .= " group by substr(order_date,6,2)";
	  if($user) $sql.= " and order_user = '$user'";
   	$query = $this->db->query($sql);
		return $query->row;
	}

	public function getTotalOrderLastMonth($user = ''){
	  $sql = "SELECT substr(order_date,6,2) as lastmonth ,SUM(order_price) AS total, SUM(payed_sum) AS payed, SUM(balance) AS balance FROM transaction WHERE substr(order_date,6,2) = substr(adddate(now(),-30),6,2)";
	  //$sql .= " group by substr(order_date,6,2)";
	  if($user) $sql.= " and order_user = '$user'";
   	$query = $this->db->query($sql);
		return $query->row;
	...
	public function getTotalSales(){
      	$query = $this->db->query("SELECT SUM(order_price) AS total FROM transaction WHERE shipped_yn = 'Y'");

		return $query->row['total'];
	}

	public function getTotalSalesByYear($year){
      	$query = $this->db->query("SELECT SUM(order_price) AS total FROM transaction WHERE YEAR(order_date) = '" . (int)$year . "'");

		return $query->row['total'];
	}
	// TU START
	public function getProductPrice($order_id, $product_id, $quantity, $default_price){
		$query = $this->db->query("SELECT customer_group_id FROM transaction WHERE order_id = '" . (int)$order_id . "'");
