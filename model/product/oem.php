<?php
class ModelProductOem extends Model{

  public function oem(){
    # oem package job list to do
    $sql = "select product_id,model from product where product_id not in (select distinct pid from product_package)";
    $query = $this->db->query($sql);
    foreach($query->rows as $row){
      $product_id = $row['product_id'];
      $model      = $row['model'];
      $modelCode  = substr($model,2,4);
      $modelPrefix = substr($model,0,2);
      $sql = "select distinct p1.product_id as base_pid from product p1, product p2
               where substr(p1.model,3,4) = substr(p2.model,3,4)
                 and substr(p1.model,3,4) = '$modelCode'
                 and substr(p1.model,1,2) in ('3S','SP','AE','VN','IR','QT')";
      //$this->log->aPrint( $sql );
      $query = $this->db->query($sql);
      
      if(isset($query->row['base_pid'])){
        $base_pid = $query->row['base_pid'];
        //$this->log->aPrint( $base_pid );
        $sql = "select * from product_package where pid = '$base_pid'";
        $query_base_pid = $this->db->query($sql);
        
        $aPP = array();
        # should find existing package from db.
        if(count($query_base_pid->rows) > 0 ){
          foreach($query_base_pid->rows as $pp){
            $pkg = $pp['pkg'];
            $pkgOem = $pkg . $modelPrefix;
            $sql = "select * from package where LCASE(code) = '$pkgOem'";
            $query2 = $this->db->query($sql);
            if(count($query2->rows) > 0 ){
              //$this->log->aPrint( $query2->rows );
              $pkg = $query2->rows[0]['code'];
      ...
      //$this->log->aPrint( $query_base_pid->rows );
      /****
      foreach($query_base_pid->rows as $pp){
        echo '<pre>';
        print_r($pp);
  ...
  public function getQuantity($model){
	...
	public function updateProduct($product_id, $data) {
	  ...
	  $sql.= " SET quantity = '" . $this->db->escape($data['quantity']) . "'";
  ...
  public function getOneData($id){
		$res = array();
		if($id){
			$sql = "SELECT * FROM product_package where id = $id ";
      $query = $this->db->query($sql);
			$rtn = $query->row;
		}
		return $rtn;
...
//print($sql);
  ...
  // todo. adhoc for OEM
	public function getProducts($data = array(),&$export_qry){
	  ...
	  $out_column = 'p.product_id,p.image,pd.name_for_sales as name,p.model,p.ws_price,p.rt_price,p.quantity,p.pc,p.status';
		if ($data) {
