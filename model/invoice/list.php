<?php
class ModelInvoiceList extends Model {
  /** deprecated
  ...
  **/
  ...
  public function print_ship($txid){
    $sql = "update transaction set print_ship = 1 where txid = '$txid'";
  ...
  public function issueInvoiceNo($data){
    ...
    if($request){
      ...
      $sql = "select x.txid, u.username as order_user, s.name as store_name,x.store_id,x.approve_status,
             ...
             x.shipped_yn, x.term, x.status,x.shipped_date,x.approved_user,x.description,x.print_ship
      ...
      if(!$this->user->isSales()){
        ...
        $sql .= " AND LCASE(x.invoice_no) = '" . $this->db->escape(strtolower($request['filter_invoice_no'])) . "'";
		  ...
		  if(isset($request['filter_ship_date']) && !is_null($request['filter_ship_date'])) {
		  	$sql .= " AND LCASE(x.shipped_date) = '" . $this->db->escape(strtolower($request['filter_ship_date'])) . "'";
		  }
